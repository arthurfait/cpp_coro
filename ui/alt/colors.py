...
class ColorChart(Frame):
    ...
    def __init__(self, root):
        ...
        lumcheck = 32768
        for color in COLORS:
            # label = Label(self, text=color, bg=color, width=17,
            #                  font=("Times", self. FONT_SIZE, "bold"))
            label = ttk.Label(self, text=color, width=17, background=color,
                             font=("Times", self. FONT_SIZE, "bold"))
            colorlum = int(sum(root. winfo_rgb(color))/3)
            if colorlum > lumcheck:
                label.configure(foreground="grey4")
            else:
                label.configure(foreground="snow")
